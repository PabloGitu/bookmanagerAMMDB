...
public class BookResource {
    ...
    /**
     * GET  /books/cover/:cover : get the books with a specific cover.
     *
     ...
     */
    @GetMapping("/books/cover/{cover}")
    public ResponseEntity<List<Book>> getBooksByCover(@PathVariable String cover, Pageable pageable) {
        log.debug("REST request to get Books with a specific cover : {}", cover);
        Page<Book> page = bookService.getBooksByCover(cover, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(page, "/api/books/cover/" + cover);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }
