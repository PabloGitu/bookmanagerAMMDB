...
export class BookService {
    ...
    // Get books with a specific cover
    findByCover(cover: string, req?: any): Observable<EntityArrayResponseType> {
        const options = createRequestOption(req);
        return this.http
            .get<IBook[]>(`${this.resourceUrl}/cover/${cover}`, { params: options, observe: 'response' })
            .pipe(map((res: EntityArrayResponseType) => this.convertDateArrayFromServer(res)));
    }
